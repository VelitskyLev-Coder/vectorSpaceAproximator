...
class FiniteVectorSpace:
    ...
    def get_orthonormal_base(self) -> list[IVector]:
        ...
        zero_vec = self.base[0] - self.base[0]
        for i, b_vec in enumerate(self.base[1:], start=1):
            orthonormal_base.append(
                (b_vec - sum([self.VEC_CLASS.inner(b_vec, a)*a for a in orthonormal_base], zero_vec)).get_normalized())
...
F = TypeVar('F', bound=Callable[..., sp.Basic])

class SimbolicFunction(IVector['SimbolicFunction']):
    def __init__(self, func: F, ver: Any):
        super().__init__()
        self.func = func
        self.ver = ver

    def __mul__(self, scalar: float) -> 'SimbolicFunction':
        return scalar*self.func

    def __add__(self, other: 'SimbolicFunction') -> 'SimbolicFunction':
        return self.func+other.func

    @staticmethod
    def inner(first: 'SimbolicFunction', second: 'SimbolicFunction') -> float:
        print(first.func, second.func, sp.integrate((first.func-second.func)**2, (first.ver, 0, 1)))
        return sp.integrate(first.func*second.func, (first.ver, 0, 1))
...
def main():
    ...
    #print('\n'.join([str(vec) for vec in W.get_orthonormal_base()]))
    x = sp.symbols('x')
    print((x-x+1)-(x-x+1))
    base = (
        SimbolicFunction(x-x+1, x),
        SimbolicFunction(x, x),
        SimbolicFunction(x**2, x),
    )
